...
class RoleCollection extends Unit {
   ...
   setRules(fn) {
    ...
    fromSide(side) {
        ...
        do {
            if (possible.size == 0) return new MafiaError("RoleCollection", "Couldn't get any role.", "RoleCollection#fromSide")
    ...
    fromAlign(side, alignment) {
      ...
      //  console.log(possible);
        do {
            if (possible.size == 0) return new MafiaError("RoleCollection", "Couldn't get any role.", "RoleCollection#fromAlign")
            const rand = possible.random();
            possible.delete(rand.name);
            if (rand.blocked == false) {
                if (rand.amount) {
