...
class Game extends EventEmitter {
    ...
    start(filler) {
        ...
        let roles;
         if (filler) roles = filler;
           ...
           this.players.forEach(val => {
            const r = roles.roleNames[Math.floor(Math.random() * roles.roleNames.length)];
    ...
    setMaj() {
        ...
        if (this.playercount % 2 == 0) {
            this.events.setMajority((this.playercount / 2) + 1);
            thing = (this.playercount / 2) + 1;
        } else { this.events.setMajority(Math.ceil(Game.playercount / 2)); thing = Math.ceil(this.playercount / 2) }
        return thing;
    }
