...
const Sheriff = new Role("Sheriff").setName("Sheriff").setSide("Town").setAlignment("Investigative").setPriority(3).isUnique(false).setAction((player, target) => {
...
const Framer = new Role("Framer").setName("Framer").setSide("Neutral").setAlignment("Evil").setPriority(2).isUnique(false).setAction((player, target) => {
	target.tags.set("framed", true);
});
Collection.addPreset(Framer);

const Escort = new Role("Escort").setName("Escort").setSide("Town").setAlignment("Support").setPriority(1).isUnique(false).setAmount(3).setAction((player, target) => {
	target.roleblocked = true;
});
const Tracker = new Role("Tracker").setSide("Town").setAlignment("Investigative").setPriority(6).isUnique(false).setAction((player, target) => {
