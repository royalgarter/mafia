...
class PriorityList {
	...
	getByPos(index, subindex) {
		if (this._priorities[index] instanceof Array) return {
			value: this._priorities[index][subindex],
			index: index,
			subindex: subindex
		};
		else return {
			value: this._priorities[index],
			index: index,
			subindex: subindex
		}
	}
	...
	find(fn) {
		let res = false;
		this.forEach((val, index, valIndex) => {
			if (fn(val, index, valIndex) && res == false) {
				res = {
					index: Number(index),
					value: val,
					subIndex: valIndex
